...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    """
    # Solution 1 erorr can not pass test case 
    # witch contain non-numeric expression like (1+'x') * 10
    ans = 0
    ...
    if isinstance(expr1, Product) and isinstance(expr2, Product):
        ans = 1
        for i in expr1: ans *= i
        for j in expr2: ans *= j
    ...
    if isinstance(expr1, Product) and isinstance(expr2, Sum):
        ans = 0
        for i in expr2: ans += i
        for j in expr1: ans *= j
